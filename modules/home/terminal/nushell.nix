{...}: {
  programs.nushell = {
    ...
    extraEnv = ''
      ...
      $env.GOPATH = (echo $env.HOME | path join "go")
      $env.NODE_PATH = (echo $env.HOME | path join ".npm-global" "lib" "node_modules")
      $env.PATH = ($env.PATH | split row (char esep) | prepend ($env.GOPATH | path join "bin"))
    '';
    configFile.text = ''
      let theme = {
        rosewater: "#f5e0dc"
